...
import DatabaseEventNode from "./Nodes/DatabaseEvent";
import DatabaseEvalEventNode from "./Nodes/DatabaseEvalEvent";
...
const App = () => {
  ...
  const nodeTypes = useMemo(
    () => ({
      ...
      database_event: DatabaseEventNode,
      database_eval_event: DatabaseEvalEventNode
    }),
    []
  );
        ...
        case "dialogue_event":
          {
            let index = 0;
            let edgeStrs = [];
            for (const opt of value.data.responses) {
              let edge = node_obj["edges"].find(edge => edge.source === value.id && edge.sourceHandle === `response_${index}`);
              let targetNode = node_obj["nodes"].find(x => x.id === edge.target);
              let fmtStr = `
  {
    display: "${opt}",
    nextStateId: "${targetNode.id}"
  }`
              edgeStrs.push(fmtStr);
              index++;
            }
            let fmtStr = `
{
  id: "${value.data.id}",
  type: "user",
  display: ["${value.data.npc_text}"],
  options: [${edgeStrs.join(",")}]
}`
            fsmStrs.push(fmtStr);
          }
          break;
            ...
            let fmtStr = `
{
  id: "${value.data.id}",
  type: "database",
  retrieveFunc: async (userId) => {
     let possible_states = {${edgeStrs.join(", ")}};
     return {
      data: await DB.query("${value.data.npc_text}"),
      nextState: "FIX_ME"
     }
  },
  displayFunc: (data) => { return ["${value.data.response_text}"] },
}`
            fsmStrs.push(fmtStr);
          }
          break;

        case "database_eval_event":
          {
            let index = 0;
            let edgeStrs = [];
            for (const opt of value.data.responses) {
            ...
            let fmtStr = `
{
  id: "${value.data.id}",
  type: "databaseEval",
  display: ["${value.data.response_text}"],
  optionFunc: async (userId) => {
    return [
      ${edgeStrs.join('\n')}
    ]
}`
    ...
    // serialize tree
    const json = jsText;// JSON.stringify(node_tree, null, 2);
    // save json to file
    const element = document.createElement("a");
    const file = new Blob([json], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = filename.replace(".dlg.src", ".ts");
