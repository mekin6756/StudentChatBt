...
export default function DatabaseEventNode({ data }) {
  // state for npc text
  const [npcText, setNpcText] = React.useState(data.npc_text || "");
  const [responseText, setResponseText] = React.useState(data.respose_text || "");
  ...
  const onChangeResponseText = (evt) => {
    setResponseText(evt.target.value);
    data.response_text = evt.target.value;
  }
  ...
  return (
    <>
      ...
      <Card className="dialogue-event">
        <NodeHeader nodeName="Database Query" id={data.id} />
        <CardContent>
          <FormLabel htmlFor="npc_text">SQL:</FormLabel>
          <Textarea
            className="margin-bottom-one"
            minRows={3}
            placeholder="Enter database SQL query here..."
            variant="soft"
            id="npc_text"
            name="npc_text"
            ...
            value={npcText}
          />
          <FormLabel htmlFor="response_text">Response Format:</FormLabel>
          <Textarea
            className="margin-bottom-one"
            minRows={3}
